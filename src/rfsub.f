...
c     the coding into an integer of the categories going left.
     ...
     3     mred, mind, fpcopy, fpind)
      implicit double precision(a-h,o-z)
      integer a(mdim,nsample), cl(nsample), cat(mdim),
     1     ncase(nsample), b(mdim,nsample), nn, j,
     1     fpind(mdim),fpindc(mdim)
      double precision tclasspop(nclass), tclasscat(nclass,53), dn(53),
     1     win(nsample), wr(nclass), wl(nclass), xrand, totalmass, mass, rT,
     1     fpcopy(mdim), fpcopyc(mdim)
...
c         nn = nn - 1
         ...
         mvar = fpindc(j)
         nn = nn - 1
         totalmass = totalmass - fpcopyc(j)
         do k = j, nn
            fpcopyc(k) = fpcopyc(k+1)
            fpindc(k) = fpindc(k+1)
         end do
