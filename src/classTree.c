...
void classTree(int *a, int *b, int *class, int *cat, int mdim, int nsample,
               ...
               int *nodeClass, int *treeSize, double *win, double *fprob) {
 ...
 */
    ...
    for (i = 0; i < numNodes; ++i) {
        ...
        F77_CALL(findbestsplit)(a, b, cl, mdim, nsample, nclass, cat,
                                ...
                                inBag, mTry, win, wr, wc, wl, mred, i, mind, fpcopy, fpind);
        if (jstat == 1) {
...
void findBestSplit(int *a, double *b, int *class, int mDim, int nSample,
                   ...
                   int *currentNode, int *mind, double *fprob) {
/*
...
*/
    ...
    *splitStatus = 0;
    ...
    //last = mDim - 1;
    int sp[mtry];
    SamplePNR(mDim,fprob,mind,mtry,sp);
