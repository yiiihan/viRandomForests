/* This program was modified by Y. Liu based on the original randomForest package */
...
void oob(int nsample, int nclass, int *cl, int *jtr,int *jerr,
         int *counttr, int *out, double *errtr, int *jest, int *jerrest, double *cutoff);
...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	     ...
	     int *outclts, int *labelts, double *proxts, double *errts,
             int *inbag, double *fprob, double *strength, double *correlation) {
    /******************************************************************
	...
	*at, *a, *b, *mind, *nind, *jts, *oobpair, *fpind;
    ...
    fpcopy =     (double *) S_alloc(mdim, sizeof(double));
    ...
    fpind =         (int *) S_alloc(mdim, sizeof(int));
    ...
    *strength = *correlation = 0.0;
    int *treecl, *treeout, *jerrest;
    treecl = (int *) S_alloc(nsample*Ntree, sizeof(int));
    zeroInt(treecl, nsample*Ntree);
    treeout = (int *) S_alloc(Ntree, sizeof(int));
    zeroInt(treeout, Ntree);
    jerrest = (int *) S_alloc(nsample, sizeof(int));
    zeroInt(jerrest, nsample);
    
    for (n=0; n< mdim; n++){
        fpcopy[n]=fprob[n];
        fpind[n]=n;
    }
    revsort(fpcopy, fpind, mdim);
    ...
    for (jb = 0; jb < Ntree; jb++) {
		...
		oob(nsample, nclass, cl, jtr, jerr, counttr, out,
			errtr + jb*(nclass+1), outcl, jerrest, cut);
    ...
    double p1k, p2k, sd = 0.0;
    int t1, t2, ntreeoob = 0;
    for (jb = 0; jb < Ntree; jb++){
        if (treeout[jb] > 0){
            ntreeoob++;
            t1 = t2 = 0;
            for (n = 0; n < nsample; n++){
                if (treecl[jb*nsample + n] == cl[n]){
                    t1++;
                } else{
                    if (treecl[jb*nsample + n] == jerrest[n]) t2++;
                }
            }
            p1k = ((double) t1)/treeout[jb];
            p2k = ((double) t2)/treeout[jb];
            sd += sqrt(p1k+p2k-(p1k-p2k)*(p1k-p2k));
        }
    }
    sd /= ntreeoob;
    ...
    for (n = 0; n < nsample; n++){
        if (out[n] > 0){
            nsampleoob++;
            mr = ((double) (counttr[n*nclass + cl[n] - 1]-counttr[n*nclass + jerrest[n] - 1])) / out[n];
            *strength += mr;
            mr2 += mr*mr;
        }
    }
    *strength /= nsampleoob;
    *correlation = (mr2/nsampleoob - (*strength) * (*strength))/(sd*sd);
...
*/
void oob(int nsample, int nclass, int *cl, int *jtr,int *jerr,
	 int *counttr, int *out, double *errtr, int *jest, int *jerrest,
	 double *cutoff) {
    int j, n, noob, *noobcl, ntie, nerrtie;
	...
	if (out[n]) {
	    ...
	    for (j = 0; j < nclass; ++j) {
	    	...
	    	if (j+1 != cl[n]){
                ...
                if (qq == smax){
                    if (unif_rand() < 1.0 / nerrtie) jerrest[n] = j+1;
                    nerrtie++;
                }
            }
