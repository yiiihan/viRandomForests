...
void regRF(double *x, double *y, int *xdim, int *sampsize,
	   ...
	   int *cat, int *maxcat, int *jprint, int *doProx, int *oobprox,
           ...
           int *nout, int *inbag, double *fprob, double *petree, double *correlation) {
    /*************************************************************************
   Input:
  ...
  *************************************************************************/
    ...
    *petree = *correlation = 0.0;
    double vartree, sdtree;
    int ntreeoob = 0;
    ...
    for (j = 0; j < *nTree; ++j) {
        ...
        *petree += vartree;
		...
		if (varImp && nOOB) {
            ...
            ///////////////////////////
            // YL comment: there are cases nOOB==0; in the original program, which does not require non-zero nOOB, delta will be nan in such case and lead to nan in the output importance score; I consider it better to remove such cases when estimating changes of mse, etc., but here for simplicity, I did not implement that (still errimp/ntree)
            ///////////////////////////
    ...
    *petree /= ntreeoob;
    sdtree /= ntreeoob;
    *correlation = errb / (sdtree*sdtree);
