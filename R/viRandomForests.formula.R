"viRandomForests.formula" <-
    function(formula, data = NULL, ..., subset, na.action = na.fail) {
### formula interface for viRandomForests.
...
###
    ...
    ## test with:
    ## viRandomForests(Fertility~.-Catholic+I(Catholic<50),data=swiss,mtry=2)
    ...
    ret <- viRandomForests.default(m, y, ...)
    cl <- match.call()
    ...
    class(ret) <- c("viRandomForests.formula", "viRandomForests")
    return(ret)
}
