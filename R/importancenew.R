importancenew <- function(x, ...)  UseMethod("importancenew")

importancenew.default <- function(x, ...)
    stop("No method implemented for this class of object")

importancenew.viRandomForests <- function(x, type=NULL, class=NULL, scale=TRUE,
                                    ...) {
    if (!inherits(x, "viRandomForests"))
        stop("x is not of class viRandomForests")
