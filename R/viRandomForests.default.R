...
"viRandomForests.default" <-
    function(x, y=NULL,  fprob = NULL, importance.sd = FALSE,
    ...
    if (is.null(fprob)){
        m1 = viRandomForests.default(x=x, y=y,  fprob = rep(1/p, p), ntree=ntree,
             mtry = mtry,
             replace=replace, classwt=classwt, cutoff=cutoff, strata=strata,
             sampsize = sampsize,
             nodesize = nodesize,
             maxnodes= maxnodes,
             importance=TRUE, importance.sd = FALSE, localImp=FALSE, nPerm=nPerm,
             proximity = FALSE, oob.prox=FALSE,
             norm.votes=norm.votes, do.trace=do.trace,
             keep.forest=FALSE, corr.bias=corr.bias,
             keep.inbag=FALSE)
        if (classRF){
             if (importance.sd == TRUE) wi = importancenew(m1)[,3] else wi = m1$importance[,3]
        } else{
             if (importance.sd == TRUE) wi = importancenew(m1)[,1] else wi = m1$importance[,1]
        }
        if (max(wi) > 0) fprob = wi/max(wi) else fprob = rep(0, p)
        fprob = 1/p + pmax(fprob, 0)
    }
    fprob = fprob / sum(fprob)
    ...
    if (classRF) {
        ...
        rfout <- .C("classRF",
			  ...
			  fprob = as.double(fprob),
			  strength = double(1),
			  correlation = double(1),
                    DUP=FALSE,
                    PACKAGE="viRandomForests")[-1]
        if (keep.forest) {
        ...
        out <- list(call = cl,
                ...
                dimnames=list(c("Test", levels(y)), NULL))) else NULL,
                    ...
                    inbag = if (keep.inbag) matrix(rfout$inbag, nrow=nrow(rfout$inbag), 
										dimnames=list(x.row.names, NULL)) else NULL, strength = rfout$strength, correlation = rfout$correlation)
    } else {
		ymean <- mean(y)
		y <- y - ymean
		ytest <- ytest - ymean
        rfout <- .C("regRF",
			  ...
			  correlation = double(1),
                    DUP=FALSE,
                    PACKAGE="viRandomForests")[c(16:28, 36:41, 43:44)]
        ...
        out <- list(call = cl,
				...
				petree = rfout$petree,
				correlation = rfout$correlation)
    }
    class(out) <- "viRandomForests"
    return(out)
}
